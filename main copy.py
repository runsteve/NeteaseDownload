...
def validateTitle(title):
    ...
    return new_title
#登录
#login_get=requests.get("http://192.168.31.111:3000/login?email=akari120@163.com&password=dwclqz123")
#login_get=requests.get("http://192.168.31.111:3000/login?email=d1203708687@163.com&password=dwclqz")
...
#歌单id
playlist_id=2782062411

playlist_detail_get=requests.get("http://192.168.31.111:3000/playlist/detail?id="+str(playlist_id))
...
while sum <= track_count:
    ...
    song_detail_get=requests.get("http://192.168.31.111:3000/song/detail?ids="+str(song_id))
    ...
    song_url_get=requests.get("http://192.168.31.111:3000/song/url?id="+str(song_id))
    song_url_json=json.loads(song_url_get.text)
    song_url=song_url_json['data'][0]['url']

    song_lyric_get=requests.get("http://192.168.31.111:3000/lyric?id="+str(song_id))
...
logout=requests.get('http://192.168.31.111:3000/logout')